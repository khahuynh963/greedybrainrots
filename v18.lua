--[[
    ===================================================================
    🧠 GREEDY BRAINROTS - ULTIMATE AUTO HUB V18 (PERFECT LIGHTNING DODGE & GROWTH TIMER)
    - Cơ Chế Né Sét Chuẩn Xác (Precision Lightning Dodge System):
      1. Đặt thời gian chờ cây lớn tối ưu (Mặc định 15s). Cây sẽ lớn liên tục trên ô Grow Pad.
      2. Theo dõi sét giáp lá hoa (mỗi 0.05s). Khi phát hiện sét, script chờ cây lớn tối đa và THU HOẠCH TRƯỚC KHI SÉT ĐÁNH TÙY CHỈNH (Mặc định ~1.0s).
      3. Nếu không có sét: Cây lớn đủ thời gian (15s) sẽ được thu hoạch tự động để trồng hạt tiếp theo.
    - Cho phép điều chỉnh Thời Gian Né Sét (~0.5s, ~0.8s, ~1.0s, ~1.2s, ~1.5s, ~2.0s) ngay trên GUI!
    - Cho phép điều chỉnh Thời Gian Chờ Cây Lớn (8s, 10s, 12s, 15s, 18s, 20s, 25s, 30s) ngay trên GUI!
    - Tự Động Chọn Đồ Ăn (Auto Select Food), My Plot Filter, Anti-Ban, Auto Buy, Auto Trash.
    ===================================================================
--]]
...
local AutoDodgeLightning = true  -- Default ON
local AutoFood = true            -- Default ON
local SelectedFoodIndex = 1      -- Default: 1 = Basic (+37% Luck Free)

-- Adjustable Lightning Dodge Lead Time (~1.0s)
local DodgeLeadTimeIndex = 3
local ALL_DODGE_TIMES = {0.5, 0.8, 1.0, 1.2, 1.5, 2.0}

-- Adjustable Max Growth Wait Time (Default 15s)
...
-- Plant tracking timestamp
local plantStartTime = 0

-- ═══════════════════════════════════════════════════════════
-- MY PLOT FINDER UTILITY
-- ═══════════════════════════════════════════════════════════
...
MainFrame.Position = UDim2.new(0.5, -165, 0.35, -290)
...
Title.Text = "🍱 BRAINROTS HUB V18 (LIGHTNING DODGE FIX)"
...
btnGrowthTime.MouseButton1Click:Connect(function()
    ...
    btnGrowthTime.Text = "⏱️ Thời Gian Chờ Cây Lớn (Nếu Không Sét): [ " .. tostring(ALL_GROWTH_TIMES[GrowthWaitIndex]) .. " Giây ]"
end)

-- 🍱 7. Auto Select Food Toggle
...
-- ═══════════════════════════════════════════════════════════
-- CORE AUTOMATION LOOPS WITH SMART LIGHTNING PROTECTION & ANTI-BAN
-- ═══════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════
-- ⚡ MULTI-LAYER LIGHTNING & TIMER DETECTOR ENGINE
-- ═══════════════════════════════════════════════════════════
local lightningKeywords = {
    "lightning", "thunder", "strike", "cloud", "bolt", 
    "warning", "danger", "beam", "electric", "flash", "spark", "fire", "timer", "storm", "sky", "weather"
}

local function checkLightningThreat(growPad, plot)
    if not growPad and not plot then return false, nil end
    
    local targetArea = growPad or plot
    local timeRemaining = nil
    local hasThreat = false

    -- Layer 1: Check Attributes for Lightning Signals or Countdown Numbers
    pcall(function()
    ...
    -- Layer 2: Check Text Labels / Guis on GrowPad & Plot for Countdown (e.g. "3.5s", "1.2s", "Lightning in 2s")
    pcall(function()
        for _, desc in pairs(targetArea:GetDescendants()) do
            if desc:IsA("TextLabel") or desc:IsA("TextButton") then
                ...
                -- If text is pure number ticking down near grow pad
            ...
            elseif desc:IsA("ParticleEmitter") or desc:IsA("Beam") or desc:IsA("Highlight") or desc:IsA("Sparkles") or desc:IsA("SelectionBox") then
    ...
    -- Layer 3: Check Workspace for nearby Lightning Objects or Effects (within 30 studs)
    if not hasThreat and growPad and growPad:IsA("BasePart") then
        pcall(function()
            for _, item in pairs(workspace:GetChildren()) do
                ...
                for _, kw in ipairs(lightningKeywords) do
                    if string.find(iName, kw) then
                        ...
                        elseif item:IsA("Model") and item.PrimaryPart then itemPos = item.PrimaryPart.Position end

                        if itemPos and (itemPos - growPad.Position).Magnitude <= 30 then
                            hasThreat = true
                            break
                        end
                    end
...
-- ═══════════════════════════════════════════════════════════
-- 🌱 ACCURATE PROXIMITY PROMPT FINDER
-- ═══════════════════════════════════════════════════════════
    ...
    for _, container in ipairs(searchList) do
        for _, desc in pairs(container:GetDescendants()) do
            if desc:IsA("ProximityPrompt") then
                ...
                local isNearMe = true
                if not myPlot and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and desc.Parent and desc.Parent:IsA("BasePart") then
                    local dist = (desc.Parent.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
                    if dist > 50 then isNearMe = false end
                end
...
-- ═══════════════════════════════════════════════════════════
-- 🌱 REVOLUTIONARY AUTO PLANT & LIGHTNING HARVEST ENGINE (State Machine - 0.05s)
-- ═══════════════════════════════════════════════════════════
task.spawn(function()
    while true do
        task.wait(0.05)
        if AutoPlant then
            pcall(function()
                local myPlot = getMyPlot()
                local plantPrompt, harvestPrompt, growPad = getGrowPadPrompts(myPlot)

                local targetDodgeLead = ALL_DODGE_TIMES[DodgeLeadTimeIndex] or 1.0
                ...
                if harvestPrompt then
                    ...
                    local hasLightning, strikeTime = checkLightningThreat(growPad, myPlot)

                    if hasLightning and AutoDodgeLightning then
                        if strikeTime then
                            if strikeTime <= targetDodgeLead then
                                setStatus("⚡ SÉT SẮP ĐÁNH (còn " .. string.format("%.1f", strikeTime) .. "s)! Thu hoạch né sét ngay!")
                                triggerPrompt(harvestPrompt)
                                task.wait(0.5)
                            else
                                setStatus("⚡ Cảnh báo sét (còn " .. string.format("%.1f", strikeTime) .. "s) - Đợi đạt size tối ưu (~" .. tostring(targetDodgeLead) .. "s)...")
                            end
                        else
                            setStatus("⚡ PHÁT HIỆN SÉT! Đang chờ 0.5s đạt size tối ưu rồi né...")
                            task.wait(0.5)
                            setStatus("⚡ THU HOẠCH NÉ SÉT NGAY LẬP TỨC!")
                            triggerPrompt(harvestPrompt)
                            task.wait(0.5)
                        end
                    else
                        -- Không có sét: Đợi cây nuôi đủ thời gian chờ (Default 15s) rồi mới harvest!
                        if elapsedTime >= targetMaxGrowthTime then
                            setStatus("🌾 Cây đã nuôi đủ " .. math.floor(elapsedTime) .. "s (Size tối đa) -> Thu hoạch!")
                            triggerPrompt(harvestPrompt)
                            task.wait(0.5)
                        else
                            setStatus("🌱 Cây đang lớn trên Grow Pad (" .. math.floor(elapsedTime) .. "s/" .. targetMaxGrowthTime .. "s)... Đang theo dõi sét ⚡")
                        end
                    end

                elseif plantPrompt then
                    -- Grow Pad rảnh -> Trồng 1 cây và ghi lại thời gian trồng
                    ...
                    plantStartTime = os.clock() -- Ghi lại mốc thời gian bắt đầu trồng
                    task.wait(0.6)
                else
                    setStatus("🔍 Đang tìm Grow Pad hoặc Nút Trồng Cây...")
                end
            end)
        end
    end
...
setStatus("Đã khởi tạo V18 - Né Sét & Thời Gian Nuôi Cây Hoàn Hảo!")
