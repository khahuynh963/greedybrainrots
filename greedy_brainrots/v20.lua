--[[
    ===================================================================
    🧠 GREEDY BRAINROTS - ULTIMATE AUTO HUB V20 (STRICT PLOT OWNERSHIP FIX)
    - Khắc Phục Triệt Để Lỗi "That's another player's plot":
      1. Thêm bộ lọc nghiêm ngặt isPromptInMyPlot(): Tự động loại bỏ TOÀN BỘ ProximityPrompt 
         thuộc về sân của người chơi khác trong server.
      2. Đảm bảo script CHỈ tương tác với nút Trồng ("Grow Brainrot", "Plant") và nút Thu Hoạch 
         nằm trên ô Grow Pad của BẠN (hoặc ngay sát vị trí nhân vật của bạn dưới 25 studs).
    - Giao diện V19 thu gọn, nút 🧠 kéo thả cảm ứng LDPlayer mượt mà.
    ===================================================================
--]]
...
Title.Text = "🍱 BRAINROTS HUB V20 (STRICT PLOT OWNERSHIP)"
...
StatusLabel.Text = "Trạng thái: Sẵn sàng. (Bấm 🧠 để Bật/Tắt Menu)"
...
-- ═══════════════════════════════════════════════════════════
-- 🌱 STRICT PLOT OWNERSHIP & PROXIMITY PROMPT CHECKER
-- ═══════════════════════════════════════════════════════════
local function isPromptInMyPlot(prompt, myPlot)
    if not prompt or not prompt:IsA("ProximityPrompt") then return false end
    
    -- 1. Must be descendant of myPlot if myPlot is resolved
    if myPlot and prompt:IsDescendantOf(myPlot) then
        return true
    end
    
    -- 2. Reject if any ancestor belongs to another player
    ...
    -- 3. Check distance to LocalPlayer Character (Must be <= 25 studs!)
    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and prompt.Parent then
        local padPos = nil
        if prompt.Parent:IsA("BasePart") then
            padPos = prompt.Parent.Position
        elseif prompt.Parent:IsA("Model") and prompt.Parent.PrimaryPart then
            padPos = prompt.Parent.PrimaryPart.Position
        end

        if padPos then
            local dist = (padPos - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude
            if dist <= 25 then
                return true
            end
        end
...
local function getGrowPadPrompts(myPlot)
    ...
    for _, container in ipairs(searchList) do
        for _, desc in pairs(container:GetDescendants()) do
            if desc:IsA("ProximityPrompt") and isPromptInMyPlot(desc, myPlot) then
...
setStatus("Đã khởi tạo V20 - Đã chặn tuyệt đối ProximityPrompt của người chơi khác!")
