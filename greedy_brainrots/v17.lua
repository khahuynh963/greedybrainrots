--[[
    ===================================================================
    🧠 GREEDY BRAINROTS - ULTIMATE AUTO HUB V17 (FIXED AUTO PLANT & LIGHTNING DODGE)
    - Khắc Phục Triệt Để Lỗi Auto Plant & Harvest:
      1. Tự động nhận diện chính xác ProximityPrompt Trồng ("Plant", "Plant Seed", "Grow Pad") 
         và Thu Hoạch ("Harvest", "Collect", "Pick", "Take") bằng thuật toán khớp từ khóa mềm.
      2. Tự động thu hoạch CẢ KHI CÂY CHÍN (Full Grown) lẫn KHI CÓ SÉT (Né sét trước ~1s).
      3. Tự động bổ sung Fallback nhận diện Plot gần nhân vật nếu My Plot chưa được gán.
    - Chu Kỳ Tự Động Hoàn Hảo: Trồng 1 Cây -> Đợi Lớn -> (Né Sét hoặc Harvest khi chín) -> Trồng Cây Tiếp Theo!
    ===================================================================
--]]
...
local AutoDodgeLightning = true  -- Harvest before lightning strikes (~1s)
local AutoFood = true            -- Auto Select Food when popup opens
...
Title.Text = "🍱 BRAINROTS HUB V17 (SMART PLANT & HARVEST)"
...
-- 3. Auto Plant Button
createToggleButton("🌱 Auto Plant (Trồng & Thu Hoạch Né Sét): OFF", Color3.fromRGB(0, 255, 170), function(btn, stroke)
    AutoPlant = not AutoPlant
    if AutoPlant then
        btn.Text = "🌱 Auto Plant (Trồng & Thu Hoạch Né Sét): ON"
        btn.TextColor3 = Color3.fromRGB(0, 255, 170)
        stroke.Color = Color3.fromRGB(0, 255, 170)
    else
        btn.Text = "🌱 Auto Plant (Trồng & Thu Hoạch Né Sét): OFF"
        btn.TextColor3 = Color3.fromRGB(220, 220, 240)
        stroke.Color = Color3.fromRGB(45, 45, 60)
    end
end)

-- 🍱 4. Auto Select Food Toggle
...
-- ═══════════════════════════════════════════════════════════
-- 🌱 HIGHLY ACCURATE PROXIMITY PROMPT FINDER
-- ═══════════════════════════════════════════════════════════
    ...
    for _, container in ipairs(searchList) do
        for _, desc in pairs(container:GetDescendants()) do
            if desc:IsA("ProximityPrompt") then
                ...
                -- Check distance to player if myPlot is not confirmed
                local isNearMe = true
                ...
                if isNearMe then
                    ...
                    -- Soft Keyword Match for Harvest
...
-- ═══════════════════════════════════════════════════════════
-- 🌱 SMART AUTO PLANT & LIGHTNING HARVEST ENGINE (State Machine - 0.05s)
-- ═══════════════════════════════════════════════════════════
task.spawn(function()
    while true do
        task.wait(0.05)
                    ...
                    if hasLightning and AutoDodgeLightning then
                        if strikeTime then
                            if strikeTime <= 1.2 then
                                ...
                                setStatus("⚡ Cảnh báo sét (còn " .. string.format("%.1f", strikeTime) .. "s) - Đợi đạt size tối ưu (~1s)...")
                            end
                        else
                            setStatus("⚡ PHÁT HIỆN SÉT! Đang đợi 0.6s đạt size rồi né...")
                            task.wait(0.6)
                        ...
                        -- Khi không có sét, cây chín (Harvest Prompt có sẵn) thì TỰ ĐỘNG THU HOẠCH để trồng cây mới!
                        setStatus("🌾 Cây đã phát triển xong -> Tự động thu hoạch!")
                        triggerPrompt(harvestPrompt)
                        task.wait(0.5)
                    end

                elseif plantPrompt then
                    -- Grow Pad rảnh -> Trồng 1 cây
                    setStatus("🌱 Đang trồng 1 hạt giống lên Grow Pad...")
...
setStatus("Đã khởi tạo V17 - Đã sửa toàn bộ logic Trồng & Thu Hoạch!")
