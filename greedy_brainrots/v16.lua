--[[
    ===================================================================
    🧠 GREEDY BRAINROTS - ULTIMATE AUTO HUB V16 (SMART AUTO PLANT & LIGHTNING DODGE)
    - Auto Plant Chuẩn Xác (1 Ô Đất Grow Pad duy nhất):
      Chỉ trồng 1 hạt giống duy nhất trên Grow Pad của BẠN, không spam trồng lung tung.
    - Né Sét Thông Minh & Thu Hoạch Tối Ưu (Lightning Dodge ~1s Before Strike):
      Theo dõi liên tục dấu hiệu sét đánh (Cảnh báo, Đếm ngược, Hiệu ứng lightning/cloud/spark/beam/timer).
      Tự động chờ cây phát triển đến kích thước tối đa và THU HOẠCH TRƯỚC KHI SÉT ĐÁNH 1 GIÂY để không bị chết cây!
    - Tự Động Chọn Đồ Ăn (Auto Select Food):
      Tự chọn đồ ăn (Basic/None/Better/Premium/Super/Magic) khi hiện bảng SELECT FOOD.
    - Nhận Diện Chính Xác Plot của Bạn (My Plot Filter).
    - Tích hợp Từ Điển Độ Hiếm (Item Database Map), Auto Buy, Auto Trash, Anti-Ban.
    ===================================================================
--]]
...
local AutoDodgeLightning = true  -- Default ON: Harvest before lightning strikes (~1s)
local AutoHarvestMax = false     -- Default OFF: Harvest when plant reaches max size without lightning
...
Title.Text = "🍱 BRAINROTS HUB V16 (SMART LIGHTNING DODGE)"
...
-- 3. Auto Plant Button
createToggleButton("🌱 Auto Plant (Trồng Ô Đất 1 Cây): OFF", Color3.fromRGB(0, 255, 170), function(btn, stroke)
    AutoPlant = not AutoPlant
    if AutoPlant then
        btn.Text = "🌱 Auto Plant (Trồng Ô Đất 1 Cây): ON"
        btn.TextColor3 = Color3.fromRGB(0, 255, 170)
        stroke.Color = Color3.fromRGB(0, 255, 170)
    else
        btn.Text = "🌱 Auto Plant (Trồng Ô Đất 1 Cây): OFF"
        btn.TextColor3 = Color3.fromRGB(220, 220, 240)
        stroke.Color = Color3.fromRGB(45, 45, 60)
    end
end)
...
-- ⚡ 6. Auto Dodge Lightning & Harvest (Thu Hoạch Né Sét Trước ~1s)
createToggleButton("⚡ Auto Né Sét ~1s Trước Khi Đánh: ON", Color3.fromRGB(255, 220, 0), function(btn, stroke)
    AutoDodgeLightning = not AutoDodgeLightning
    if AutoDodgeLightning then
        btn.Text = "⚡ Auto Né Sét ~1s Trước Khi Đánh: ON"
        btn.TextColor3 = Color3.fromRGB(255, 220, 0)
        stroke.Color = Color3.fromRGB(255, 220, 0)
    else
        btn.Text = "⚡ Auto Né Sét ~1s Trước Khi Đánh: OFF"
        btn.TextColor3 = Color3.fromRGB(220, 220, 240)
        stroke.Color = Color3.fromRGB(45, 45, 60)
    end
end)

-- 🌽 7. Auto Harvest Optimal Size (Thu Hoạch Tối Ưu Khi Đạt Size)
createToggleButton("🌽 Auto Harvest Tối Ưu (Khi Đạt Size): OFF", Color3.fromRGB(0, 220, 130), function(btn, stroke)
    AutoHarvestMax = not AutoHarvestMax
    if AutoHarvestMax then
        btn.Text = "🌽 Auto Harvest Tối Ưu (Khi Đạt Size): ON"
        btn.TextColor3 = Color3.fromRGB(0, 220, 130)
        stroke.Color = Color3.fromRGB(0, 220, 130)
    else
        btn.Text = "🌽 Auto Harvest Tối Ưu (Khi Đạt Size): OFF"
        btn.TextColor3 = Color3.fromRGB(220, 220, 240)
        stroke.Color = Color3.fromRGB(45, 45, 60)
    end
end)

-- 🌾 8. Instant Harvest All Button
...
-- ═══════════════════════════════════════════════════════════
-- ⚡ LIGHTNING DETECTOR & TIMING UTILITY
-- ═══════════════════════════════════════════════════════════
local lightningKeywords = {
    "lightning", "thunder", "strike", "cloud", "bolt", 
    "warning", "danger", "beam", "electric", "flash", "spark", "fire", "timer"
}
...
local function checkLightningThreat(growPad, plot)
    ...
    -- 1. Check Attributes on GrowPad / Plot
    pcall(function()
    ...
    -- 2. Check Children & Descendants for Visuals or Timer Text
    pcall(function()
        for _, desc in pairs(targetArea:GetDescendants()) do
            ...
            elseif desc:IsA("ParticleEmitter") or desc:IsA("Beam") or desc:IsA("Highlight") or desc:IsA("Sparkles") then
    ...
    -- 3. Check Workspace for nearby Lightning Effects / Beams (within 25 studs of Grow Pad)
            ...
            for _, item in pairs(workspace:GetChildren()) do
                ...
                for _, kw in ipairs(lightningKeywords) do
                    if string.find(iName, kw) then
                        ...
                        if itemPos and (itemPos - growPad.Position).Magnitude <= 25 then
                            hasThreat = true
                            break
                        end
                    end
...
-- ═══════════════════════════════════════════════════════════
-- 🌱 SMART AUTO PLANT & LIGHTNING HARVEST ENGINE (State Machine Loop - 0.05s)
-- ═══════════════════════════════════════════════════════════
task.spawn(function()
    while true do
        task.wait(0.05)
        if AutoPlant then
            pcall(function()
                local myPlot = getMyPlot()
                if not myPlot then
                    setStatus("🔍 Đang tìm ô đất (Plot) của bạn...")
                    return
                end

                -- Tìm duy nhất 1 Ô Đất Grow Pad trên Plot của Bạn
                local growPad = nil
                ...
                for _, desc in pairs(myPlot:GetDescendants()) do
                    if desc:IsA("ProximityPrompt") then
                        ...
                        -- Prompt trồng cây
                        if act == "plant" or string.find(obj, "grow pad") and not (string.find(act, "harvest") or string.find(act, "collect") or string.find(act, "pick")) then
                            if act == "plant" then
                                plantPrompt = desc
                                growPad = desc.Parent
                            end
                ...
                -- Quyết định trạng thái (State Machine):
                if harvestPrompt then
                    -- Đang có cây lớn trên Grow Pad!
                    local hasLightning, strikeTime = checkLightningThreat(growPad, myPlot)

                    if hasLightning and AutoDodgeLightning then
                        if strikeTime then
                            if strikeTime <= 1.2 then
                                setStatus("⚡ SÉT SẮP ĐÁNH (còn " .. string.format("%.1f", strikeTime) .. "s)! Thu hoạch né sét ngay!")
                                triggerPrompt(harvestPrompt)
                                task.wait(0.4)
                            else
                                setStatus("⚡ Phát hiện sét (còn " .. string.format("%.1f", strikeTime) .. "s) - Đợi đạt size tối ưu (~1s)...")
                            end
                        else
                            -- Cảnh báo sét không có đếm ngược số: Chờ 0.8s cho cây phát triển tối đa rồi né!
                            setStatus("⚡ DẤU HIỆU SÉT! Đang đợi 0.8s đạt size tối ưu...")
                            task.wait(0.8)
                    ...
                    elseif AutoHarvestMax then
                        setStatus("🌽 Cây đã chín - Thu hoạch tối ưu!")
                        triggerPrompt(harvestPrompt)
                        task.wait(0.4)
                    else
                        setStatus("🌱 Cây đang lớn trên Grow Pad... (Đang theo dõi sét ⚡)")
                    end

                elseif plantPrompt then
                    -- Ô Grow Pad đang trống! Chỉ trồng đúng 1 hạt giống
                    setStatus("🌱 Đang trồng 1 hạt giống mới lên Grow Pad...")

                    -- Trang bị Ungrown Tool từ Backpack nếu chưa cầm trên tay
                    ...
                    task.wait(0.6) -- Tạm dừng 0.6s sau khi trồng để không bị spam
                else
                    setStatus("🌱 Không thấy Grow Pad rảnh trên Plot của bạn...")
                end
            end)
        end
    end
...
setStatus("Đã khởi tạo V16 - Auto Plant 1 Ô & Né Sét ~1s Chuẩn!")
